...
import models.jointemb, configs    
...
def repr_code(args):
    ...
    use_set = eval(config['dataset_name'])(data_path, 
								   ...
								   config['use_graphseq'],config['graphseq_len'])

    data_loader = torch.utils.data.DataLoader(dataset=use_set, batch_size=args.batch_size,shuffle=False, drop_last=False, num_workers=1)
...
def parse_args():
    ...
    parser.add_argument('--batch_size', type=int, default=1, help='how many instances for encoding and normalization at each step')
